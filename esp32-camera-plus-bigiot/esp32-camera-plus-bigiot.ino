    ...
    if (1) { 
        ...
        if (!fb) {
            ...
            if (now - last_upload_time > UPLOAD_TIMESTAMP) {
             ...
             uint16_t mcu_y= tft.height();
             uint16_t mcu_x= tft.width();
             int block_grey_16_8 = 0;
             for(uint16_t i=0;i<mcu_y;i++){
             for(uint16_t j=0;j<mcu_x;j++){
             //Serial.println(*(d_buffer+i*240+j));
             uint16_t ewha=*(d_buffer+i*240+j);
             //Serial.print("Block value is : ");
             
             block_grey_16_8=(ewha>>11)*0.2126+
                                ((ewha&0x7c0)>>5)*0.7152+
                                (ewha&0x1f)*0.0722;                                
              ...
              Serial.println(block_grey_16_8);
             } 
             }
            }
